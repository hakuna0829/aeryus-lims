...
import { updatePassword } from 'actions/api';
...
const ResetPassword = props => {
  ...
  const handleClickShowConformPassword = () => setShowConformPassword(!showConformPassword);
  const handleMouseDownConformPassword = () => setShowConformPassword(!showConformPassword);
  ...
  const handleSubmit = async event => {
    ...
    if (formState.isValid) {
      ...
      let res = await updatePassword(token, body);
      setLoading(false);
      if (res.success) {
...
export default connect(mapStateToProps, { updatePassword, clearErrors })(withRouter(ResetPassword));
